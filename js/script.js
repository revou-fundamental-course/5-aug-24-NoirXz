...
const BMI_CATEGORIES = {
  ...
  // Hitung BMI berdasarkan berat badan dan tinggi badan
  ...
  // Validasi input berat badan, tinggi badan, usia, dan jenis kelamin
  const validateInput = (weight, height, age, gender) => {
    ...
    if (isNaN(weight) || weight <= 0) {
      weightErrorMessage.innerText = 'Berat badan harus berupa angka dan lebih dari 0';
      isValid = false;
    }
  
    // Validasi tinggi badan
    if (isNaN(height) || height <= 0) {
      heightErrorMessage.innerText = 'Tinggi badan harus berupa angka dan lebih dari 0';
      isValid = false;
    }
  
    // Validasi usia
    if (isNaN(age) || age <= 0) {
      ageErrorMessage.innerText = 'Umur harus berupa angka dan lebih dari 0';
      isValid = false;
    }
  ...
  // Menampilkan hasil BMI, status, saran, dan resiko penyakit
  const generateDisplay = (bmi, status) => {
    ...
    resultDesc.innerText = getDeskripsi(status);
  
    const resultText = document.getElementById('result-text');
    resultText.innerText = 'Hasil BMI: ${bmi}';
  
    const suggestionText = document.getElementById('suggestion-text');
    suggestionText.innerText = getSuggestion(status);
  
    const adviceText = document.getElementById('advice-text');
    adviceText.innerText = getSaran(status);
    ...
    const diseases = getPenyakit(status);
